...
eval $(ps -aux --sort=-%mem -h | awk -v num=0 \
    '{if($3<50){exit} else{num++;printf("memid["num"]=%d;", $2)}} END{printf("memnum=%d;",num)}')

if [[ ${cpunum} -gt 0 || ${memnum} -get 0 ]]; then
    sleep 5
else
    exit 0
fi
...
if [[ ${cpunum} -gt 0 ]]; then
    ...
    for i in ${cpupid[*]}; do
        eval $(`ps -aux -h -q ${i} | awk -v num=${cnt}\
            '{if ($3<50){exit} else{printf("Pname["num"]=%s;Pid["num"]=%d;User["num"]=%d;CpuP["num"]=%.2f;MemP["num"]=%.2f;",$11,$2,$1,$3,$4)} }'`)
        cnt=$[ ${cnt} + 1 ]
    done
fi
